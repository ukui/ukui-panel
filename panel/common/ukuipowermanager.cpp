...
#include "ukuipowermanager.h"
#include "ukuipower/ukuipower.h"
...
class UKUI_API MessageBox: public QMessageBox
{
    Q_DECLARE_TR_FUNCTIONS(UKUi::MessageBox)
...
public:
    ...
    static void warning(const QString& title, const QString& text)
    {
        Q_UNUSED(title)
        Q_UNUSED(text)
        QMessageBox::warning(parentWidget(), tr("UKUi Power Manager Error"), tr("Hibernate failed."));
    }
...
//            this, SLOT(monitoring(const QString&)));
    ...
    QString sessionConfig(QFile::decodeName(qgetenv("UKUI_SESSION_CONFIG")));
...
{
     if (m_skipWarning ||
         MessageBox::question(tr("UKUi Session Suspend"),
...
{
    if (m_skipWarning ||
        MessageBox::question(tr("UKUi Session Hibernate"),
...
{
    if (m_skipWarning ||
        MessageBox::question(tr("UKUi Session Reboot"),
...
{
    if (m_skipWarning ||
        MessageBox::question(tr("UKUi Session Shutdown"),
...
{
    if (m_skipWarning ||
        MessageBox::question(tr("UKUi Session Logout"),
...
{
    MessageBox::warning(tr("UKUi Power Manager Error"), tr("Hibernate failed."));
}

void PowerManager::suspendFailed()
{
    MessageBox::warning(tr("UKUi Power Manager Error"), tr("Suspend failed."));
}
