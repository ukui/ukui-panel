...
bool XdgDesktopFileData::startByDBus(const QString & action, const QStringList& urls) const
{
    ...
    // see discussion https://github.com/ukui/libqtxdg/pull/75
...
//    for (const QString &dirName : std::as_const(dataDirs))
    ...
    {
        const QString &dirName=dataDirs[i];
...
//    for (const QString &mimeDir : std::as_const(mimeDirsList))
    for (int i=0;i<mimeDirsList.size();i++)
    {
        const QString &mimeDir=mimeDirsList[i];
