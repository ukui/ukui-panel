...
setup_actions (Launcher *launcher)
{
	...
	for(; *ptr != NULL; ptr++) {
		...
		ukui_panel_applet_add_callback (launcher->info,
						callback,
						NULL,
						action_name,
						NULL);
...
setup_button (Launcher *launcher)
{
	...
	ukui_panel_applet_clear_user_menu (launcher->info);
		      ...
		      const char       *id)
{
	...
	launcher->info = ukui_panel_applet_register (launcher->button, launcher,
						free_launcher,
			      ...
			      const char  *id)
{
	...
	if (launcher) {
		if (!g_settings_is_writable (settings, PANEL_OBJECT_LAUNCHER_LOCATION_KEY)) {
			...
			menu = ukui_panel_applet_get_callback (launcher->info->user_menu,
							  "properties");
			if (menu != NULL)
			    ...
			    const char    *location)
{
	...
	if (strstr (filename, "desktop")) {
		WriteAppletInfo ("delete", filename, "after");
	}
...
find_launcher (const char *path)
{
	...
	for (l = ukui_panel_applet_list_applets (); l; l = l->next) {
