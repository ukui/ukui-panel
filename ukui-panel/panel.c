/* Ukui panel: Initialization routines
		    ...
		    PanelWidget *panel)
{
	...
	case PANEL_OBJECT_APPLET:
		ukui_panel_applet_frame_change_orientation (
				UKUI_PANEL_APPLET_FRAME (info->widget), orientation);
		break;
	     ...
	     PanelWidget *panel)
{
	if (info->type == PANEL_OBJECT_APPLET)
		ukui_panel_applet_frame_change_size (
			UKUI_PANEL_APPLET_FRAME (info->widget), panel->sz);
}
	     ...
	     PanelWidget *panel)
{
	switch (info->type) {
	case PANEL_OBJECT_APPLET:
		ukui_panel_applet_frame_change_background (

#if GTK_CHECK_VERSION (3, 18, 0)
		UKUI_PANEL_APPLET_FRAME (info->widget), panel->toplevel->background.type);
#else
		UKUI_PANEL_APPLET_FRAME (info->widget), panel->background.type);
#endif
		break;
	case PANEL_OBJECT_MENU_BAR:
...
static void
ukui_panel_applet_added(GtkWidget *widget, GtkWidget *applet, gpointer data)
{
	AppletInfo    *info;
...
static void
ukui_panel_applet_removed(GtkWidget *widget, GtkWidget *applet, gpointer data)
{
...
static void
ukui_panel_applet_move(PanelWidget *panel, GtkWidget *widget, gpointer data)
{
	AppletInfo *info;
	...
	ukui_panel_applet_save_position (info, info->id, FALSE);
}
			   ...
			   const char  *uri)
{
	...
	if (strncmp (basename, "trash", strlen ("trash")) == 0)
		ukui_panel_applet_frame_create (panel->toplevel, pos,
					   "OAFIID:UKUI_Panel_TrashApplet");
		...
		if (info) {
			...
			if (mime &&
				...
				if (!set_background_image_from_uri (panel->toplevel, uri))
				   ...
				   (!strcmp (mime, "application/x-ukui-app-info") ||
...
move_applet (PanelWidget *panel, int pos, int applet_index)
{
	...
	applet_list = ukui_panel_applet_list_applets ();

	info = g_slist_nth_data (applet_list, applet_index);

	if ( ! ukui_panel_applet_can_freely_move (info))
		return FALSE;
	...
	if (info != NULL &&
		...
		forb = g_object_get_data (G_OBJECT (info->widget),
					  UKUI_PANEL_APPLET_FORBIDDEN_PANELS);
...
get_target_list (void)
{
	static GtkTargetEntry drop_types [] = {
		...
		{ "application/x-ukui-panel-applet-iid",      0, TARGET_APPLET },
		{ "application/x-ukui-panel-applet-internal", 0, TARGET_APPLET_INTERNAL },
			...
			guint             time_)
{
	...
	case TARGET_APPLET:
		...
		if (panel_profile_id_lists_are_writable ()) {
			ukui_panel_applet_frame_create (panel->toplevel, pos, (char *) data);
			success = TRUE;
		} else {
			success = FALSE;
		}
		       ...
		       guint             time)
{
	...
	/* 
	 * -1 passed to ukui_panel_applet_register will turn on 
...
panel_widget_setup(PanelWidget *panel)
{
	g_signal_connect (G_OBJECT(panel),
			  "applet_added",
			  G_CALLBACK(ukui_panel_applet_added),
			  NULL);
	g_signal_connect (G_OBJECT(panel),
			  "applet_removed",
			  G_CALLBACK(ukui_panel_applet_removed),
			  NULL);
	g_signal_connect (G_OBJECT(panel),
			  "applet_move",
			  G_CALLBACK(ukui_panel_applet_move),
			  NULL);
	g_signal_connect (G_OBJECT (panel),
			  "back_change",
