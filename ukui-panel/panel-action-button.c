...
/* taken from deprecated ukui-conf code */
static const gchar*
...
{
	ukui_panel_applet_add_callback (button->priv->info,
				   "activate",
				   NULL,
...
panel_action_search (GtkWidget *widget)
{
	...
	panel_launch_desktop_file_with_fallback ("ukui-search-tool.desktop",
						 "ukui-search-tool",
						 screen, NULL);
}
			  ...
			  const char            *id)
{
	...
	button->priv->info = ukui_panel_applet_register (GTK_WIDGET (button),
						    NULL, NULL,
				 ...
				 const char        *callback_name)
{
	...
	if (!strcmp (callback_name, "help")) {
		...
		panel_show_help (screen, "ukui-user-guide",
