...
static gboolean ukui_panel_applet_in_drag = FALSE;
	...
	for(list = panel->applet_list;list!=NULL;list = g_list_next(list)) {
		...
		PanelWidget *p =
			g_object_get_data (G_OBJECT(ad->applet),
					   UKUI_PANEL_APPLET_ASSOC_PANEL_KEY);
		if(p)
...
panel_widget_cremove (GtkContainer *container, GtkWidget *widget)
{
	...
	ad = g_object_get_data (G_OBJECT (widget), UKUI_PANEL_APPLET_DATA);
	p = g_object_get_data (G_OBJECT (widget),
				 UKUI_PANEL_APPLET_ASSOC_PANEL_KEY);
			      ...
			      GtkWidget   *widget)
{
	...
	applet = g_object_get_data (G_OBJECT (widget), UKUI_PANEL_APPLET_DATA);
	if (applet) {
		GList *applet_list, *l;
		int    end_pos = -1;
					...
					int drag_off)
{
	...
	ad = g_object_get_data (G_OBJECT (applet), UKUI_PANEL_APPLET_DATA);
	...
	if (info != NULL &&
	    ! ukui_panel_applet_can_freely_move (info))
		return FALSE;
	...
	ukui_panel_applet_in_drag = TRUE;
...
panel_widget_applet_drag_end_no_grab (PanelWidget *panel)
{
	...
	ukui_panel_applet_in_drag = FALSE;
...
panel_sub_event_handler(GtkWidget *widget, GdkEvent *event, gpointer data)
{
	...
	switch (event->type) {
		...
		case GDK_MOTION_NOTIFY: {
			...
			if (bevent->button != 1 || ukui_panel_applet_in_drag)
...
panel_widget_applet_destroy (GtkWidget *applet, gpointer data)
{
	...
	ad = g_object_get_data (G_OBJECT (applet), UKUI_PANEL_APPLET_DATA);
	g_object_set_data (G_OBJECT (applet), UKUI_PANEL_APPLET_DATA, NULL);
		       ...
		       int pos)
{
	 ...
	 * https://github.com/ukui-desktop/ukui-panel/issues/504
	 */
	gtk_widget_reparent (applet, GTK_WIDGET (new_panel));

	if (info && info->type == PANEL_OBJECT_APPLET)
		ukui_panel_applet_frame_set_panel (UKUI_PANEL_APPLET_FRAME (ad->applet), new_panel);
...
gboolean
ukui_panel_applet_is_in_drag (void)
{
	return ukui_panel_applet_in_drag;
}
