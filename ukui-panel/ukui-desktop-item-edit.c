...
#include "applet.h"
GSList *ukui_panel_applet_list_applets (void) { return NULL; }
...
main (int argc, char * argv[])
{
	...
	if (desktops == NULL ||
	    desktops[0] == NULL) {
		g_printerr ("ukui-desktop-item-edit: no file to edit\n");
		return 0;
	}
		...
		if (info) {
			...
			if (type == G_FILE_TYPE_DIRECTORY && create_new) {
				...
				dlg = panel_ditem_editor_new (NULL, NULL, uri,
				   ...
				   && create_new) {
				g_printerr ("ukui-desktop-item-edit: %s "
					    "already exists\n", uri);
			} else {
				g_printerr ("ukui-desktop-item-edit: %s "
			...
			g_printerr ("ukui-desktop-item-edit: %s does not "
