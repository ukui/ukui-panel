/* Ukui panel: general applet functionality
...
ukui_panel_applet_get_panel_widget (AppletInfo *info)
{
	return PANEL_WIDGET (gtk_widget_get_parent (info->widget));
}

static void
ukui_panel_applet_set_dnd_enabled (AppletInfo *info,
...
gboolean
ukui_panel_applet_toggle_locked (AppletInfo *info)
{
	PanelWidget *panel_widget;
	gboolean     locked;

	panel_widget = ukui_panel_applet_get_panel_widget (info);

	locked = panel_widget_toggle_applet_locked (panel_widget, info->widget);

	ukui_panel_applet_save_position (info, info->id, TRUE);
	ukui_panel_applet_set_dnd_enabled (info, !locked);

	return locked;
}

static void
ukui_panel_applet_lock (GtkCheckMenuItem *menuitem,
		   AppletInfo       *info)
{
	gboolean locked;

	locked = ukui_panel_applet_toggle_locked (info);
 ...
 * cleanups should go in ukui_panel_applet_destroy()
 */
void
ukui_panel_applet_clean (AppletInfo *info)
{
	g_return_if_fail (info != NULL);
...
static void
ukui_panel_applet_recreate_menu (AppletInfo	*info)
{
	GList *l;
	...
	info->menu = ukui_panel_applet_create_menu (info);
}

static void
ukui_panel_applet_locked_change_notify (GSettings *settings,
									    gchar *key,
									    GtkWidget   *applet)
{
	gboolean     locked;
	...
	ukui_panel_applet_toggle_locked (info);

	if (info->type == PANEL_OBJECT_APPLET)
		ukui_panel_applet_frame_sync_menu_state (UKUI_PANEL_APPLET_FRAME (info->widget));
	else
		ukui_panel_applet_recreate_menu (info);
}
...
applet_user_menu_get_screen (AppletUserMenu *menu)
{
	...
	panel_widget = ukui_panel_applet_get_panel_widget (menu->info);
	...
	case PANEL_OBJECT_DRAWER:
		if (strcmp (menu->name, "add") == 0) {
			...
			panel_show_help (screen,
					 "ukui-user-guide", "gospanel-18", NULL);
		}
		break;
	case PANEL_OBJECT_MENU:
...
AppletUserMenu *
ukui_panel_applet_get_callback (GList      *user_menu,
			   const char *name)
{
	GList *l;
...
void
ukui_panel_applet_add_callback (AppletInfo          *info,
			   ...
			   CallbackEnabledFunc  is_enabled_func)
{
	...
	g_return_if_fail (info != NULL);
	g_return_if_fail (ukui_panel_applet_get_callback (info->user_menu,
						     callback_name) == NULL);
...
void
ukui_panel_applet_clear_user_menu (AppletInfo *info)
{
	GList *l;
		 ...
		 GList *user_menu)
{
	...
	*p = '\0';
	...
	s_menu = ukui_panel_applet_get_callback (user_menu, t);
...
GtkWidget *
ukui_panel_applet_create_menu (AppletInfo *info)
{
	GtkWidget   *menu;
	GtkWidget   *menuitem;
	GList       *l;
	...
	if (!panel_lockdown_get_locked_down ()) {
		...
		lockable = ukui_panel_applet_lockable (info);
		movable = ukui_panel_applet_can_freely_move (info);
		...
		g_signal_connect (menuitem, "toggled",
				  G_CALLBACK (ukui_panel_applet_lock), info);
		gtk_widget_show (menuitem);
		gtk_menu_shell_append (GTK_MENU_SHELL (menu), menuitem);
...
void
ukui_panel_applet_menu_set_recurse (GtkMenu     *menu,
			       const gchar *key,
			       gpointer     data)
{
	GList *children;
	...
	for (l = children; l; l = l->next) {
		...
		if (submenu)
			ukui_panel_applet_menu_set_recurse (
		  ...
		  GdkEventButton *event)
{
	...
	ukui_panel_applet_menu_set_recurse (GTK_MENU (info->menu),
				       "menu_panel",
				       panel_widget);
...
applet_do_popup_menu (GtkWidget      *widget,
		      ...
		      AppletInfo     *info)
{
	if (ukui_panel_applet_is_in_drag ())
		return FALSE;
		     ...
		     AppletInfo     *info)
{
	...
	if (!applet_locked) ukui_panel_applet_set_dnd_enabled (info, TRUE);
...
static void
ukui_panel_applet_destroy (GtkWidget  *widget,
		      AppletInfo *info)
{
	g_return_if_fail (info != NULL);
	...
	if (info->type != PANEL_OBJECT_APPLET)
		panel_lockdown_notify_remove (G_CALLBACK (ukui_panel_applet_recreate_menu),
					      info);
	...
	ukui_panel_applet_clear_user_menu (info);
...
typedef struct {
	...
	guint            right_stick : 1;
	guint            locked : 1;
} UkuiPanelAppletToLoad;

/* Each time those lists get both empty,
 * ukui_panel_applet_queue_initial_unhide_toplevels() should be called */
static GSList  *ukui_panel_applets_to_load = NULL;
static GSList  *ukui_panel_applets_loading = NULL;
/* We have a timeout to always unhide toplevels after a delay, in case of some
 * blocking applet */
#define         UNHIDE_TOPLEVELS_TIMEOUT_SECONDS 5
static guint    ukui_panel_applet_unhide_toplevels_timeout = 0;

static gboolean ukui_panel_applet_have_load_idle = FALSE;

static void
free_applet_to_load (UkuiPanelAppletToLoad *applet)
{
	g_free (applet->id);
	applet->id = NULL;
...
gboolean
ukui_panel_applet_on_load_queue (const char *id)
{
	GSList *li;
	for (li = ukui_panel_applets_to_load; li != NULL; li = li->next) {
		UkuiPanelAppletToLoad *applet = li->data;
		if (strcmp (applet->id, id) == 0)
			return TRUE;
	}
	for (li = ukui_panel_applets_loading; li != NULL; li = li->next) {
		UkuiPanelAppletToLoad *applet = li->data;
		if (strcmp (applet->id, id) == 0)
			return TRUE;
	}
...
static gboolean
ukui_panel_applet_queue_initial_unhide_toplevels (gpointer user_data)
{
	GSList *l;

	if (ukui_panel_applet_unhide_toplevels_timeout != 0) {
		g_source_remove (ukui_panel_applet_unhide_toplevels_timeout);
		ukui_panel_applet_unhide_toplevels_timeout = 0;
	}
...
void
ukui_panel_applet_stop_loading (const char *id)
{
	UkuiPanelAppletToLoad *applet;
	GSList *l;

	for (l = ukui_panel_applets_loading; l; l = l->next) {
		applet = l->data;
	 ...
	 * for example */
	if (l != NULL) {
		ukui_panel_applets_loading = g_slist_delete_link (ukui_panel_applets_loading, l);
		free_applet_to_load (applet);
	}

	if (ukui_panel_applets_loading == NULL && ukui_panel_applets_to_load == NULL)
		ukui_panel_applet_queue_initial_unhide_toplevels (NULL);
}
...
ukui_panel_applet_load_idle_handler (gpointer dummy)
{
	...
	for (l = ukui_panel_applets_to_load; l; l = l->next) {
		applet = l->data;
	...
	if (!l) {
		...
		g_slist_free (ukui_panel_applets_to_load);
		ukui_panel_applets_to_load = NULL;
		ukui_panel_applet_have_load_idle = FALSE;

		if (ukui_panel_applets_loading == NULL) {
	...
	ukui_panel_applets_to_load = g_slist_delete_link (ukui_panel_applets_to_load, l);
	ukui_panel_applets_loading = g_slist_append (ukui_panel_applets_loading, applet);
	 ...
	 * ukui_panel_applet_stop_loading() for this type. However, in case of
	 * failure during the load, we might call ukui_panel_applet_stop_loading()
	...
	switch (applet_type) {
	case PANEL_OBJECT_APPLET:
		ukui_panel_applet_frame_load_from_gsettings (
					panel_widget,
					applet->locked,
					applet->position,
					applet->id);
	...
	if (applet_type != PANEL_OBJECT_APPLET)
		ukui_panel_applet_stop_loading (applet->id);

	return TRUE;
}

void
ukui_panel_applet_queue_applet_to_load (const char      *id,
				   PanelObjectType  type,
				   ...
				   gboolean         locked)
{
	...
	applet = g_new0 (UkuiPanelAppletToLoad, 1);
	...
	ukui_panel_applets_to_load = g_slist_prepend (ukui_panel_applets_to_load, applet);
}

static int
ukui_panel_applet_compare (const UkuiPanelAppletToLoad *a,
		      const UkuiPanelAppletToLoad *b)
{
	int c;
...
void
ukui_panel_applet_load_queued_applets (gboolean initial_load)
{
	if (!ukui_panel_applets_to_load) {
		ukui_panel_applet_queue_initial_unhide_toplevels (NULL);
		return;
	...
	if (ukui_panel_applets_to_load && ukui_panel_applet_unhide_toplevels_timeout == 0) {
		/* Install a timeout to make sure we don't block the
		 * unhiding because of an applet that doesn't load */
		ukui_panel_applet_unhide_toplevels_timeout =
			g_timeout_add_seconds (UNHIDE_TOPLEVELS_TIMEOUT_SECONDS,
					       ukui_panel_applet_queue_initial_unhide_toplevels,
					       NULL);
	}

	ukui_panel_applets_to_load = g_slist_sort (ukui_panel_applets_to_load,
					      (GCompareFunc) ukui_panel_applet_compare);

	if ( ! ukui_panel_applet_have_load_idle) {
...
static const char* ukui_panel_applet_get_toplevel_id(AppletInfo* applet)
{
	PanelWidget* panel_widget;
	...
	panel_widget = ukui_panel_applet_get_panel_widget(applet);
...
static gboolean
ukui_panel_applet_position_save_timeout (gpointer dummy)
{
	GSList *l;
...
void
ukui_panel_applet_save_position (AppletInfo *applet_info,
			    const char *id,
			    gboolean    immediate)
{
	PanelWidget       *panel_widget;
		...
		if (!queued_position_source)
			queued_position_source =
				g_timeout_add_seconds (1,
						       (GSourceFunc) ukui_panel_applet_position_save_timeout,
						       NULL);
	...
	if (!(toplevel_id = ukui_panel_applet_get_toplevel_id (applet_info)))
		return;

	panel_widget = ukui_panel_applet_get_panel_widget (applet_info);
	...
	position = ukui_panel_applet_get_position (applet_info);
...
const char *
ukui_panel_applet_get_id (AppletInfo *info)
{
	if (!info)
		return NULL;
...
const char *
ukui_panel_applet_get_id_by_widget (GtkWidget *applet_widget)
{
	GSList *l;
...
AppletInfo *
ukui_panel_applet_get_by_id (const char *id)
{
	GSList *l;
...
GSList *
ukui_panel_applet_list_applets (void)
{
	return registered_applets;
}

AppletInfo *
ukui_panel_applet_get_by_type (PanelObjectType object_type, GdkScreen *screen)
{
	GSList *l;
...
AppletInfo *
ukui_panel_applet_register (GtkWidget       *applet,
		       gpointer         data,
		       ...
		       const char      *id)
{
	...
	if (type != PANEL_OBJECT_APPLET)
		panel_lockdown_notify_add (G_CALLBACK (ukui_panel_applet_recreate_menu),
					   info);

	locked_changed = g_strdup_printf ("changed::%s", PANEL_OBJECT_LOCKED_KEY);
	g_signal_connect (info->settings,
					  locked_changed,
					  G_CALLBACK (ukui_panel_applet_locked_change_notify),
					  G_OBJECT (applet));
	...
	if (type == PANEL_OBJECT_DRAWER) {
		...
		g_object_set_data (G_OBJECT (applet),
				   UKUI_PANEL_APPLET_ASSOC_PANEL_KEY, assoc_panel);
	...
	g_object_set_data (G_OBJECT (applet),
			   UKUI_PANEL_APPLET_FORBIDDEN_PANELS, NULL);
	...
	g_signal_connect (applet, "destroy",
			  G_CALLBACK (ukui_panel_applet_destroy),
			  info);
...
int
ukui_panel_applet_get_position (AppletInfo *applet)
{
	...
	applet_data = g_object_get_data (G_OBJECT (applet->widget), UKUI_PANEL_APPLET_DATA);

	return applet_data->pos;
}

gboolean
ukui_panel_applet_can_freely_move (AppletInfo *applet)
{
	if (panel_lockdown_get_locked_down ())
		return FALSE;
...
gboolean
ukui_panel_applet_lockable (AppletInfo *applet)
{
	if (panel_lockdown_get_locked_down ())
		return FALSE;
