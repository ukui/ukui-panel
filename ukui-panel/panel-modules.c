...
panel_modules_ensure_extension_points_registered (void)
{
	...
	if (!registered_extensions) {
		...
		ep = g_io_extension_point_register (UKUI_PANEL_APPLETS_MANAGER_EXTENSION_POINT_NAME);
...
panel_modules_ensure_loaded (void)
{
	...
	if (!loaded_dirs) {
		...
		module_path = g_getenv ("UKUI_PANEL_EXTRA_MODULES");
		...
		ukui_panel_applets_manager_dbus_get_type ();
	}
}
