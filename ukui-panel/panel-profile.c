...
panel_profile_get_attached_object_settings (PanelToplevel *toplevel)
{
	...
	id = ukui_panel_applet_get_id_by_widget (attach_widget);
...
#define UPDATE_CENTERED(k, n, n2)                                   \
	...
	else UPDATE_BOOL ("enable-animations", aniukui)
...
#define GET_ENUM(k, fn)                                           \
	...
	GET_BOOL ("enable-animations", aniukui);
...
panel_profile_load_and_show_toplevel (char *toplevel_id)
{
	...
	if (!loading_queued_applets)
		ukui_panel_applet_load_queued_applets (FALSE);
...
panel_profile_delete_object (AppletInfo *applet_info)
{
	...
	id = ukui_panel_applet_get_id (applet_info);
...
panel_profile_load_object (char *id)
{
	...
	ukui_panel_applet_queue_applet_to_load (id,
					   object_type,
					   toplevel_id,
					   position,
					   right_stick,
...
panel_profile_object_id_list_update (gchar **objects)
{
	...
	existing_applets = ukui_panel_applet_list_applets ();
	...
	panel_profile_load_added_ids (sublist,
								  object_ids,
								  (PanelProfileGetIdFunc) ukui_panel_applet_get_id,
								  (PanelProfileLoadFunc) panel_profile_load_object,
								  (PanelProfileOnLoadQueue) ukui_panel_applet_on_load_queue);
...
{
	profile_settings = g_settings_new ("org.ukui.panel");
}
...
panel_profile_load (void)
{
	...
	ukui_panel_applet_load_queued_applets (TRUE);
}
