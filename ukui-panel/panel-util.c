...
static char* panel_lock_screen_action_get_command(const char* action)
{
	...
	if (panel_is_program_in_path("ukui-screensaver-command") && panel_is_program_in_path("ukui-screensaver-preferences"))
	{
	...
	if (strcmp (action, "prefs") == 0)
	{
		if (use_gscreensaver)
		{
			command = g_strdup ("ukui-screensaver-preferences");
		}
	...
	{
		/* Neither ukui-screensaver or xscreensaver allow root
		 * to lock the screen */
		if (geteuid () == 0)
		{
			command = NULL;
		...
		else
		{
			if (use_gscreensaver)
			{
				command = g_strdup_printf("ukui-screensaver-command --%s", action);
			}
			else
			{
...
void panel_lock_screen_action(GdkScreen* screen, const char* action)
{
	...
	if (!ukui_gdk_spawn_command_line_on_screen(screen, command, &error))
	{
...
{
		return g_build_filename(g_get_user_config_dir(), "ukui", "panel2.d", "default", "launchers", NULL);
}
...
panel_util_get_icon_for_uri (const char *text_uri)
{
	 ...
	 *  + check for application/x-ukui-saved-search mime type and override
	 *    icon of the GFile
