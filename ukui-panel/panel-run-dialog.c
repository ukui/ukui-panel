...
#define PANEL_RUN_SCHEMA "org.ukui.panel"
#define PANEL_RUN_HISTORY_KEY "history-ukui-run"
#define PANEL_RUN_HISTORY_MAX_SIZE_KEY "history-max-size-ukui-run"
#define PANEL_RUN_HISTORY_REVERSE_KEY "history-reverse-ukui-run"
			   ...
			   GtkWidget      *run_dialog)
{
	...
	case GTK_RESPONSE_HELP:
		panel_show_help (gtk_window_get_screen (GTK_WINDOW (run_dialog)),
				 "ukui-user-guide", "gospanel-23", NULL);
		break;
	default:
		break;
	}
...
static int
compare_applications (UkuiMenuTreeEntry *a,
		      UkuiMenuTreeEntry *b)
{
	return g_utf8_collate (ukuimenu_tree_entry_get_display_name (a),
			       ukuimenu_tree_entry_get_display_name (b));
}

static GSList *get_all_applications_from_dir (UkuiMenuTreeDirectory *directory,
					      GSList            *list);

static GSList *
get_all_applications_from_alias (UkuiMenuTreeAlias *alias,
				 GSList         *list)
{
	...
	case UKUIMENU_TREE_ITEM_ENTRY:
		list = g_slist_append (list, ukuimenu_tree_item_ref (aliased_item));
		break;
...
static GSList *
get_all_applications_from_dir (UkuiMenuTreeDirectory *directory,
			       GSList             *list)
{
	GSList *items;
	GSList *l;
	...
	for (l = items; l; l = l->next) {
		...
		case UKUIMENU_TREE_ITEM_ENTRY:
			list = g_slist_append (list, ukuimenu_tree_item_ref (l->data));
			break;
...
panel_run_dialog_add_items_idle (PanelRunDialog *dialog)
{
	...
	for (l = all_applications; l; l = next) {
		...
		entry_name = ukuimenu_tree_entry_get_display_name (entry);
	...
	for (l = all_applications; l; l = l->next) {
		UkuiMenuTreeEntry *entry = l->data;
		GtkTreeIter    iter;
		const gchar *icon = ukuimenu_tree_entry_get_icon (entry);
		...
		gtk_list_store_set (dialog->program_list_store, &iter,
				    COLUMN_GICON,     gicon,
				    COLUMN_NAME,      ukuimenu_tree_entry_get_display_name (entry),
				    COLUMN_COMMENT,   ukuimenu_tree_entry_get_comment (entry),
				    COLUMN_EXEC,      ukuimenu_tree_entry_get_exec (entry),
				    COLUMN_PATH,      ukuimenu_tree_entry_get_desktop_file_path (entry),
				    COLUMN_VISIBLE,   TRUE,
				    -1);
