					  ...
					  gboolean       for_end_position)
{
	...
	/* For the initial animation, we aniukui from outside the screen, and
				   ...
				   gboolean       resize)
{
	...
	if(resize || move)
	{
		for(list = toplevel->priv->panel_widget->applet_list;list!=NULL;list = g_list_next(list)) 
		{
			...
			id = ukui_panel_applet_get_id_by_widget (ad->applet);

			if (!id)
				return;

			AppletInfo *info;
			info = ukui_panel_applet_get_by_id (id);
		     ...
		     GtkDirectionType  direction)
{
	...
	if (toplevel->priv->aniukui && gtk_widget_get_realized (GTK_WIDGET (toplevel)))
			     ...
			     GParamSpec   *pspec)
{
	...
	case PROP_AUTOHIDE_SIZE:
		...
		break;
	case PROP_ANIUKUI:
		panel_toplevel_set_aniukui (toplevel, g_value_get_boolean (value));
		break;
			     ...
			     GParamSpec *pspec)
{
	...
	case PROP_AUTOHIDE_SIZE:
		...
		break;
	case PROP_ANIUKUI:
		g_value_set_boolean (value, toplevel->priv->aniukui);
		break;
	case PROP_ANIMATION_SPEED:
...
panel_toplevel_class_init (PanelToplevelClass *klass)
{
	...
	g_object_class_install_property (
		...
		g_param_spec_enum (
			"animation-speed",
			"Animation Speed",
			"The speed at which to aniukui panel hiding/showing",
...
void
panel_toplevel_set_aniukui (PanelToplevel *toplevel,
			    gboolean       aniukui)
{
	g_return_if_fail (PANEL_IS_TOPLEVEL (toplevel));

	aniukui = aniukui != FALSE;

	if (toplevel->priv->aniukui == aniukui)
		return;

	toplevel->priv->aniukui = aniukui;

	g_object_notify (G_OBJECT (toplevel), "aniukui");
}

gboolean
panel_toplevel_get_aniukui (PanelToplevel *toplevel)
{
	g_return_val_if_fail (PANEL_IS_TOPLEVEL (toplevel), FALSE);

	return toplevel->priv->aniukui;
}
