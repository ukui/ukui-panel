		     ...
		     GtkWidget    *applet_window)
{
	...
	if (!ukui_panel_applet_container_add_finish (UKUI_PANEL_APPLET_CONTAINER (source_object),
						res, &error)) {
		GtkWidget *dialog;
			 ...
			 guint       orientation)
{
	...
	ukui_panel_applet_container_add (UKUI_PANEL_APPLET_CONTAINER (container),
...
setup_options (void)
{
	UkuiPanelAppletsManager *manager;
	GList               *applet_list, *l;
	int                  i;
	int                  j;
	...
	applet_list = UKUI_PANEL_APPLETS_MANAGER_GET_CLASS (manager)->get_applets (manager);
	for (l = applet_list, i = 1; l; l = g_list_next (l), i++) {
		UkuiPanelAppletInfo *info = (UkuiPanelAppletInfo *)l->data;

		gtk_list_store_append (model, &iter);
		gtk_list_store_set (model, &iter,
				    COLUMN_TEXT, g_strdup (ukui_panel_applet_info_get_name (info)),
				    COLUMN_ITEM, g_strdup (ukui_panel_applet_info_get_iid (info)),
				    -1);
	}
...
main (int argc, char **argv)
{
	...
	if (g_file_test ("../libukui-panel-applet", G_FILE_TEST_IS_DIR)) {
		applets_dir = g_strdup_printf ("%s:../libukui-panel-applet", UKUI_PANEL_APPLETS_DIR);
		g_setenv ("UKUI_PANEL_APPLETS_DIR", applets_dir, FALSE);
		g_free (applets_dir);
	}
