...
static MenuPathRootItem root_items[] = {
	{APPLICATIONS_MENU, "ukui-applications", "ukui-applications.menu"},
	{SETTINGS_MENU, "ukui-settings", "ukui-settings.menu"}
};
...
panel_menu_button_associate_panel (PanelMenuButton *button)
{
	...
	ukui_panel_applet_menu_set_recurse (GTK_MENU (button->priv->menu), "menu_panel", panel_widget);
}
			...
			const char  *id)
{
	...
	info = ukui_panel_applet_register (GTK_WIDGET (button), NULL, NULL,
	...
	ukui_panel_applet_add_callback (info, "help", GTK_STOCK_HELP, _("_Help"), NULL);

        if (!panel_lockdown_get_locked_down () &&
            (panel_is_program_in_path ("mozo") ||
	    panel_is_program_in_path ("menulibre")))
		ukui_panel_applet_add_callback (info, "edit", NULL,
					   _("_Edit Menus"), NULL);
...
panel_menu_button_get_icon (PanelMenuButton *button)
{
	...
	if (!retval                     &&
		...
		if (directory)
			retval = g_strdup (ukuimenu_tree_directory_get_icon (directory));
	}
			       ...
			       const char   *callback_name)
{
	...
	if (!strcmp (callback_name, "help")) {
		panel_show_help (screen, "ukui-user-guide", "gospanel-37", NULL);
