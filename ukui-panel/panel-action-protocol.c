/*
 * panel-action-protocol.h: _UKUI_PANEL_ACTION protocol impl.
 *
...
static Atom atom_ukui_panel_action            = None;
static Atom atom_gnome_panel_action           = None;
static Atom atom_ukui_panel_action_main_menu  = None;
static Atom atom_ukui_panel_action_run_dialog = None;
static Atom atom_gnome_panel_action_main_menu  = None;
static Atom atom_gnome_panel_action_run_dialog = None;
static Atom atom_ukui_panel_action_kill_dialog = None;
				 ...
				 guint32    activate_time)
{
	...
	info = ukui_panel_applet_get_by_type (PANEL_OBJECT_MENU_BAR, screen);
	if (info) {
	...
	info = ukui_panel_applet_get_by_type (PANEL_OBJECT_MENU, screen);
			      ...
			      gpointer   data)
{
	...
	if ((xevent->xclient.message_type != atom_ukui_panel_action) &&
	...
	if (xevent->xclient.data.l [0] == atom_ukui_panel_action_main_menu)
		panel_action_protocol_main_menu (screen, xevent->xclient.data.l [1]);
	else if (xevent->xclient.data.l [0] == atom_ukui_panel_action_run_dialog)
		...
		panel_action_protocol_run_dialog (screen, xevent->xclient.data.l [1]);
	else if (xevent->xclient.data.l [0] == atom_ukui_panel_action_kill_dialog)
...
panel_action_protocol_init (void)
{
	...
	atom_gnome_panel_action =
		XInternAtom (GDK_DISPLAY_XDISPLAY (display),
			     "_GNOME_PANEL_ACTION",
			     FALSE);
	atom_ukui_panel_action_main_menu =
		XInternAtom (GDK_DISPLAY_XDISPLAY (display),
			     "_UKUI_PANEL_ACTION_MAIN_MENU",
			     FALSE);
	atom_ukui_panel_action_run_dialog =
		XInternAtom (GDK_DISPLAY_XDISPLAY (display),
			     "_UKUI_PANEL_ACTION_RUN_DIALOG",
			     FALSE);
	...
	atom_gnome_panel_action_run_dialog =
		XInternAtom (GDK_DISPLAY_XDISPLAY (display),
			     ...
			     FALSE);
	atom_ukui_panel_action_kill_dialog =
		XInternAtom (GDK_DISPLAY_XDISPLAY (display),
			     "_UKUI_PANEL_ACTION_KILL_DIALOG",
			     FALSE);
