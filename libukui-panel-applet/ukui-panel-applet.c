/*
 * ukui-panel-applet.c: panel applet writing library.
 *
...
#include "ukui-panel-applet-factory.h"
#include "ukui-panel-applet-marshal.h"
#include "ukui-panel-applet-enums.h"
#if GTK_CHECK_VERSION (3, 18, 0)
#include "panel-plug-private.h"
#endif
#define UKUI_PANEL_APPLET_GET_PRIVATE(o) (G_TYPE_INSTANCE_GET_PRIVATE ((o), PANEL_TYPE_APPLET, UkuiPanelAppletPrivate))

struct _UkuiPanelAppletPrivate {
	GtkWidget         *plug;
	GtkWidget         *applet;
...
static guint ukui_panel_applet_signals[LAST_SIGNAL] = { 0 };
...
static void       ukui_panel_applet_handle_background   (UkuiPanelApplet       *applet);
static GtkAction *ukui_panel_applet_menu_get_action     (UkuiPanelApplet       *applet,
						    const gchar       *action);
static void       ukui_panel_applet_menu_update_actions (UkuiPanelApplet       *applet);
static void       ukui_panel_applet_menu_cmd_remove     (GtkAction         *action,
						    UkuiPanelApplet       *applet);
static void       ukui_panel_applet_menu_cmd_move       (GtkAction         *action,
						    UkuiPanelApplet       *applet);
static void       ukui_panel_applet_menu_cmd_lock       (GtkAction         *action,
						    UkuiPanelApplet       *applet);
static void       ukui_panel_applet_register_object     (UkuiPanelApplet       *applet);
void	_ukui_panel_applet_apply_css	(GtkWidget* widget, UkuiPanelAppletBackgroundType type);
...
static const GtkActionEntry menu_entries[] = {
	{ "Remove", GTK_STOCK_REMOVE, N_("_Remove From Panel"),
	  NULL, NULL,
	  G_CALLBACK (ukui_panel_applet_menu_cmd_remove) },
	{ "Move", NULL, N_("_Move"),
	  NULL, NULL,
	  G_CALLBACK (ukui_panel_applet_menu_cmd_move) }
};

static const GtkToggleActionEntry menu_toggle_entries[] = {
	{ "Lock", NULL, N_("Loc_k To Panel"),
	  NULL, NULL,
	  G_CALLBACK (ukui_panel_applet_menu_cmd_lock) }
};

G_DEFINE_TYPE (UkuiPanelApplet, ukui_panel_applet, GTK_TYPE_EVENT_BOX)

#define UKUI_PANEL_APPLET_INTERFACE   "org.ukui.panel.applet.Applet"
#define UKUI_PANEL_APPLET_OBJECT_PATH "/org/ukui/panel/applet/%s/%d"

char *
ukui_panel_applet_get_preferences_path (UkuiPanelApplet *applet)
{
...
static void
ukui_panel_applet_set_preferences_path (UkuiPanelApplet *applet,
...
{
	g_return_val_if_fail (PANEL_IS_APPLET (applet), UKUI_PANEL_APPLET_FLAGS_NONE);
...
static void
ukui_panel_applet_size_hints_ensure (UkuiPanelApplet *applet,
				int          new_size)
{
...
static gboolean
ukui_panel_applet_size_hints_changed (UkuiPanelApplet *applet,
			     ...
			     int          base_size)
{
	...
	if (!ukui_panel_applet_size_hints_changed (applet, size_hints, n_elements, base_size))
		return;

	ukui_panel_applet_size_hints_ensure (applet, n_elements);
	for (i = 0; i < n_elements; i++)
...
static void
ukui_panel_applet_set_size (UkuiPanelApplet *applet,
		       guint        size)
{
	g_return_if_fail (PANEL_IS_APPLET (applet));
	...
	g_signal_emit (G_OBJECT (applet),
		       ukui_panel_applet_signals [CHANGE_SIZE],
		       0, size);
...
static void
ukui_panel_applet_set_orient (UkuiPanelApplet      *applet,
			 UkuiPanelAppletOrient orient)
{
	g_return_if_fail (PANEL_IS_APPLET (applet));
	...
	g_signal_emit (G_OBJECT (applet),
		       ukui_panel_applet_signals [CHANGE_ORIENT],
		       0, orient);
...
static void
ukui_panel_applet_set_locked (UkuiPanelApplet *applet,
			 gboolean     locked)
{
	GtkAction *action;
	...
	action = ukui_panel_applet_menu_get_action (applet, "Lock");
	g_signal_handlers_block_by_func (action,
					 ukui_panel_applet_menu_cmd_lock,
					 applet);
	...
	ukui_panel_applet_menu_update_actions (applet);
...
static void
ukui_panel_applet_set_locked_down (UkuiPanelApplet *applet,
			      gboolean     locked_down)
{
	g_return_if_fail (PANEL_IS_APPLET (applet));
...
static void
ukui_panel_applet_init_atoms (Display *xdisplay)
{
...
static Window
ukui_panel_applet_find_toplevel_dock_window (UkuiPanelApplet *applet,
					Display	    *xdisplay)
{
	GtkWidget  *toplevel;
	Window	    xwin;
			    ...
			    guint32	  timestamp)
{
	...
	ukui_panel_applet_init_atoms (xdisplay);

	dock_xwindow = ukui_panel_applet_find_toplevel_dock_window (applet, xdisplay);
...
static GtkAction *
ukui_panel_applet_menu_get_action (UkuiPanelApplet *applet,
			      const gchar *action)
{
...
static void
ukui_panel_applet_menu_update_actions (UkuiPanelApplet *applet)
{
	...
	g_object_set (ukui_panel_applet_menu_get_action (applet, "Move"),
		      ...
		      "visible", !locked_down,
		      NULL);
	g_object_set (ukui_panel_applet_menu_get_action (applet, "Remove"),
		      "sensitive", !locked,
		      "visible", !locked_down,
		      NULL);
}
...
static void
ukui_panel_applet_menu_cmd_remove (GtkAction   *action,
			      UkuiPanelApplet *applet)
{
	GError *error = NULL;
...
static void
ukui_panel_applet_menu_cmd_move (GtkAction   *action,
			    UkuiPanelApplet *applet)
{
	GError *error = NULL;
...
static void
ukui_panel_applet_menu_cmd_lock (GtkAction   *action,
			    UkuiPanelApplet *applet)
{
	gboolean locked;

	locked = gtk_toggle_action_get_active (GTK_TOGGLE_ACTION (action));
	ukui_panel_applet_set_locked (applet, locked);
}
			 ...
			 GtkActionGroup *applet_action_group)
{
	...
	new_xml = g_strdup_printf ("<ui><popup name=\"UkuiPanelAppletPopup\" action=\"AppletItems\">"
...
void
ukui_panel_applet_setup_menu_from_file (UkuiPanelApplet    *applet,
				   ...
				   GtkActionGroup *applet_action_group)
{
	...
	if (g_file_get_contents (filename, &xml, NULL, &error)) {
		ukui_panel_applet_setup_menu (applet, xml, applet_action_group);
	} else {
...
static void
ukui_panel_applet_finalize (GObject *object)
{
	UkuiPanelApplet *applet = UKUI_PANEL_APPLET (object);
	...
	ukui_panel_applet_set_preferences_path (applet, NULL);
	...
	G_OBJECT_CLASS (ukui_panel_applet_parent_class)->finalize (object);
}
...
static void
ukui_panel_applet_position_menu (GtkMenu   *menu,
			    int       *x,
			    int       *y,
			    gboolean  *push_in,
			    GtkWidget *widget)
{
	...
	applet = UKUI_PANEL_APPLET (widget);
	...
	if (applet->priv->orient == UKUI_PANEL_APPLET_ORIENT_UP ||
	    applet->priv->orient == UKUI_PANEL_APPLET_ORIENT_DOWN) {
...
static void
ukui_panel_applet_menu_popup (UkuiPanelApplet *applet,
			 guint        button,
			 guint32      time)
{
	GtkWidget *menu;
...
static gboolean
ukui_panel_applet_can_focus (GtkWidget *widget)
{
	/*
	 * A UkuiPanelApplet widget can focus if it has a tooltip or it does
...
/* Taken from libukuicomponentui/ukuicomponent/ukuicomponent-plug.c */
static gboolean
ukui_panel_applet_button_event (UkuiPanelApplet      *applet,
...
static gboolean
ukui_panel_applet_button_press (GtkWidget      *widget,
			   GdkEventButton *event)
{
	UkuiPanelApplet *applet = UKUI_PANEL_APPLET (widget);
	...
	if (event->button == 3) {
		ukui_panel_applet_menu_popup (applet, event->button, event->time);

		return TRUE;
	}

	return ukui_panel_applet_button_event (applet, event);
}

static gboolean
ukui_panel_applet_button_release (GtkWidget      *widget,
			     GdkEventButton *event)
{
	UkuiPanelApplet *applet = UKUI_PANEL_APPLET (widget);
...
static gboolean
ukui_panel_applet_popup_menu (GtkWidget *widget)
{
	ukui_panel_applet_menu_popup (UKUI_PANEL_APPLET (widget), 3, GDK_CURRENT_TIME);
...
#if !GTK_CHECK_VERSION (3, 18, 0)
static void
ukui_panel_applet_get_preferred_width (GtkWidget *widget,
				       int       *minimum_width,
				       int       *natural_width)
{
	int focus_width = 0;

	GTK_WIDGET_CLASS (ukui_panel_applet_parent_class)->get_preferred_width (widget,
										minimum_width,
										natural_width);
	if (!ukui_panel_applet_can_focus (widget))
		return;
...
static void
ukui_panel_applet_get_preferred_height (GtkWidget *widget,
					int       *minimum_height,
					int       *natural_height)
{
	int focus_width = 0;

	GTK_WIDGET_CLASS (ukui_panel_applet_parent_class)->get_preferred_height (widget,
										 minimum_height,
										 natural_height);
	if (!ukui_panel_applet_can_focus (widget))
		return;
...
ukui_panel_applet_get_request_mode (GtkWidget *widget)
{
	UkuiPanelApplet *applet = UKUI_PANEL_APPLET (widget);
	UkuiPanelAppletOrient orientation;

	orientation = ukui_panel_applet_get_orient (applet);
...
static void
ukui_panel_applet_size_allocate (GtkWidget     *widget,
			    GtkAllocation *allocation)
{
	...
	if (!ukui_panel_applet_can_focus (widget)) {
		GTK_WIDGET_CLASS (ukui_panel_applet_parent_class)->size_allocate (widget, allocation);
	} else {
#if !GTK_CHECK_VERSION (3, 18, 0)
		/*
		 * We are deliberately ignoring focus-padding here to
	...
	if (applet->priv->previous_height != allocation->height ||
		...
		ukui_panel_applet_handle_background (applet);
	}
}

static gboolean ukui_panel_applet_draw(GtkWidget* widget, cairo_t* cr)
{
	GtkStyleContext *context;
	int border_width;
#if !GTK_CHECK_VERSION (3, 18, 0)
	...
	GTK_WIDGET_CLASS (ukui_panel_applet_parent_class)->draw(widget, cr);
...
static gboolean
ukui_panel_applet_focus (GtkWidget        *widget,
		    GtkDirectionType  dir)
{
	...
	ret = GTK_WIDGET_CLASS (ukui_panel_applet_parent_class)->focus (widget, dir);
...
static gboolean
ukui_panel_applet_parse_color (const gchar *color_str,
			       GdkRGBA     *color)
{
...
static gboolean
ukui_panel_applet_parse_pixmap_str (const char *str,
			       Window          *xid,
			       int             *x,
			       int             *y)
{
...
ukui_panel_applet_create_foreign_surface_for_display (GdkDisplay *display,
                                                      GdkVisual  *visual,
                                                      Window      xid)
{
	Status result = 0;
...
ukui_panel_applet_get_pattern_from_pixmap (UkuiPanelApplet *applet,
			 Window           xid,
			 int              x,
			 int              y)
{
	...
	background = ukui_panel_applet_create_foreign_surface_for_display (gdk_window_get_display (window),
...
static UkuiPanelAppletBackgroundType
ukui_panel_applet_handle_background_string (UkuiPanelApplet  *applet,
					    GdkRGBA          *color,
					    cairo_pattern_t **pattern)
{
	UkuiPanelAppletBackgroundType   retval;
	char                          **elements;
	...
	if (elements [0] && !strcmp (elements [0], "none" )) {
		...
		if (!elements [1] || !ukui_panel_applet_parse_color (elements [1], color)) {
		...
		if (!ukui_panel_applet_parse_pixmap_str (elements [1], &pixmap_id, &x, &y)) {
		...
		*pattern = ukui_panel_applet_get_pattern_from_pixmap (applet, pixmap_id, x, y);
		if (!*pattern) {
				  ...
				  cairo_pattern_t **pattern)
{
	...
	return ukui_panel_applet_handle_background_string (applet, color, pattern);
}

static void
ukui_panel_applet_set_background_string (UkuiPanelApplet *applet,
...
static void
ukui_panel_applet_handle_background (UkuiPanelApplet *applet)
{
	...
	type = ukui_panel_applet_get_background (applet, &color, &pattern);

	switch (type) {
	case PANEL_NO_BACKGROUND:
		g_signal_emit (G_OBJECT (applet),
			       ukui_panel_applet_signals [CHANGE_BACKGROUND],
...
static void
ukui_panel_applet_realize (GtkWidget *widget)
{
	GTK_WIDGET_CLASS (ukui_panel_applet_parent_class)->realize (widget);

	if (UKUI_PANEL_APPLET (widget)->priv->background)
		ukui_panel_applet_handle_background (UKUI_PANEL_APPLET (widget));
}

static void
ukui_panel_applet_move_focus_out_of_applet (UkuiPanelApplet      *applet,
				       GtkDirectionType  dir)
{
	GtkWidget *toplevel;
...
static void
ukui_panel_applet_change_background(UkuiPanelApplet *applet,
				    UkuiPanelAppletBackgroundType type,
				    GdkRGBA* color,
				    cairo_pattern_t *pattern)
{
	...
	if (applet->priv->out_of_process)
		...
		if (applet->priv->out_of_process)
			_ukui_panel_applet_apply_css(GTK_WIDGET(applet->priv->plug),type);
		switch (type) {
		case PANEL_NO_BACKGROUND:
			if (applet->priv->out_of_process){
...
static void
ukui_panel_applet_get_property (GObject    *object,
			   guint       prop_id,
			   GValue     *value,
			   GParamSpec *pspec)
{
...
static void
ukui_panel_applet_set_property (GObject      *object,
			   guint         prop_id,
			   const GValue *value,
			   GParamSpec   *pspec)
{
	...
	case PROP_CLOSURE:
		...
		g_closure_set_marshal (applet->priv->closure,
				       ukui_panel_applet_marshal_BOOLEAN__STRING);
		break;
	case PROP_CONNECTION:
		applet->priv->connection = g_value_dup_object (value);
		break;
	case PROP_PREFS_PATH:
		ukui_panel_applet_set_preferences_path (applet, g_value_get_string (value));
		break;
	case PROP_ORIENT:
		ukui_panel_applet_set_orient (applet, g_value_get_uint (value));
		break;
	case PROP_SIZE:
		ukui_panel_applet_set_size (applet, g_value_get_uint (value));
		break;
	case PROP_BACKGROUND:
		ukui_panel_applet_set_background_string (applet, g_value_get_string (value));
		break;
	case PROP_FLAGS:
		ukui_panel_applet_set_flags (applet, g_value_get_uint (value));
		break;
	case PROP_SIZE_HINTS: {
		const int *size_hints;
		gsize      n_elements;
		...
		ukui_panel_applet_set_size_hints (applet, size_hints, n_elements, 0);
	}
		break;
	case PROP_LOCKED:
		ukui_panel_applet_set_locked (applet, g_value_get_boolean (value));
		break;
	case PROP_LOCKED_DOWN:
		ukui_panel_applet_set_locked_down (applet, g_value_get_boolean (value));
		break;
	default:
		G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
	}
...
static void
ukui_panel_applet_setup (UkuiPanelApplet *applet)
{
	GValue   value = {0, };
	GArray  *params;
	gint     i;
...
void _ukui_panel_applet_apply_css(GtkWidget* widget, UkuiPanelAppletBackgroundType type)
{
	...
	switch (type) {
	case PANEL_NO_BACKGROUND:
		gtk_style_context_remove_class (context, "ukui-custom-panel-background");
		break;
	case PANEL_COLOR_BACKGROUND:
	case PANEL_PIXMAP_BACKGROUND:
		gtk_style_context_add_class (context, "ukui-custom-panel-background");
		break;
	default:
		g_assert_not_reached ();
		break;
...
static void _ukui_panel_applet_prepare_css (GtkStyleContext *context)
{
...
static void
ukui_panel_applet_init (UkuiPanelApplet *applet)
{
	applet->priv = UKUI_PANEL_APPLET_GET_PRIVATE (applet);

	applet->priv->flags  = UKUI_PANEL_APPLET_FLAGS_NONE;
	applet->priv->orient = UKUI_PANEL_APPLET_ORIENT_UP;
	applet->priv->size   = 24;
...
static GObject *
ukui_panel_applet_constructor (GType                  type,
                          ...
                          GObjectConstructParam *construct_properties)
{
	...
	object = G_OBJECT_CLASS (ukui_panel_applet_parent_class)->constructor (type,
	                                                                  n_construct_properties,
	                                                                  construct_properties);
	applet = UKUI_PANEL_APPLET (object);
	...
	_ukui_panel_applet_prepare_css(context);

	g_signal_connect_swapped (G_OBJECT (applet->priv->plug), "embedded",
		                      G_CALLBACK (ukui_panel_applet_setup),
		                      applet);
...
static void
ukui_panel_applet_constructed (GObject* object)
{
	UkuiPanelApplet* applet = UKUI_PANEL_APPLET(object);

	/* Rename the class to have compatibility with all GTK2 themes
	 * https://github.com/perberos/Ukui-Desktop-Environment/issues/27
	 */
	gtk_widget_set_name(GTK_WIDGET(applet), "PanelApplet");

	ukui_panel_applet_register_object (applet);
}

static void
ukui_panel_applet_class_init (UkuiPanelAppletClass *klass)
{
	GObjectClass   *gobject_class = (GObjectClass *) klass;
	GtkWidgetClass *widget_class = (GtkWidgetClass *) klass;
	GtkBindingSet *binding_set;

	gobject_class->get_property = ukui_panel_applet_get_property;
	gobject_class->set_property = ukui_panel_applet_set_property;
	gobject_class->constructor = ukui_panel_applet_constructor;
	gobject_class->constructed = ukui_panel_applet_constructed;
	klass->move_focus_out_of_applet = ukui_panel_applet_move_focus_out_of_applet;
	klass->change_background = ukui_panel_applet_change_background;
	widget_class->button_press_event = ukui_panel_applet_button_press;
	widget_class->button_release_event = ukui_panel_applet_button_release;
	widget_class->get_request_mode = ukui_panel_applet_get_request_mode;
#if !GTK_CHECK_VERSION (3, 18, 0)
	widget_class->get_preferred_width = ukui_panel_applet_get_preferred_width;
	widget_class->get_preferred_height = ukui_panel_applet_get_preferred_height;
#endif
	widget_class->draw = ukui_panel_applet_draw;
	widget_class->size_allocate = ukui_panel_applet_size_allocate;
	widget_class->focus = ukui_panel_applet_focus;
	widget_class->realize = ukui_panel_applet_realize;
	widget_class->popup_menu = ukui_panel_applet_popup_menu;

	gobject_class->finalize = ukui_panel_applet_finalize;

	g_type_class_add_private (klass, sizeof (UkuiPanelAppletPrivate));
	...
	g_object_class_install_property (gobject_class,
					 ...
					 g_param_spec_uint ("orient",
							    ...
							    UKUI_PANEL_APPLET_ORIENT_FIRST,
							    UKUI_PANEL_APPLET_ORIENT_LAST,
	...
	ukui_panel_applet_signals [CHANGE_ORIENT] =
                g_signal_new ("change_orient",
                              ...
                              G_SIGNAL_RUN_LAST,
                              G_STRUCT_OFFSET (UkuiPanelAppletClass, change_orient),
                              NULL,
			      NULL,
                              ukui_panel_applet_marshal_VOID__UINT,
                              G_TYPE_NONE,
			      1,
			      G_TYPE_UINT);
	...
	ukui_panel_applet_signals [CHANGE_SIZE] =
                g_signal_new ("change_size",
                              G_TYPE_FROM_CLASS (klass),
                              G_SIGNAL_RUN_LAST,
                              G_STRUCT_OFFSET (UkuiPanelAppletClass, change_size),
                              NULL,
			      NULL,
                              ukui_panel_applet_marshal_VOID__INT,
                              G_TYPE_NONE,
			      1,
			      G_TYPE_INT);
	...
	ukui_panel_applet_signals [CHANGE_BACKGROUND] =
                g_signal_new ("change_background",
                              G_TYPE_FROM_CLASS (klass),
                              G_SIGNAL_RUN_LAST,
                              G_STRUCT_OFFSET (UkuiPanelAppletClass, change_background),
                              NULL,
			      NULL,
                              ukui_panel_applet_marshal_VOID__ENUM_BOXED_OBJECT,
                              G_TYPE_NONE,
			      3,
			      PANEL_TYPE_UKUI_PANEL_APPLET_BACKGROUND_TYPE,
			      GDK_TYPE_RGBA,
			      CAIRO_GOBJECT_TYPE_PATTERN);

	ukui_panel_applet_signals [MOVE_FOCUS_OUT_OF_APPLET] =
                g_signal_new ("move_focus_out_of_applet",
                              G_TYPE_FROM_CLASS (klass),
                              G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                              G_STRUCT_OFFSET (UkuiPanelAppletClass, move_focus_out_of_applet),
                              NULL,
			      NULL,
                              ukui_panel_applet_marshal_VOID__ENUM,
                              G_TYPE_NONE,
			      1,
...
{
	UkuiPanelApplet* applet = g_object_new(PANEL_TYPE_APPLET, NULL);
...
method_call_cb (GDBusConnection       *connection,
		...
		gpointer               user_data)
{
	UkuiPanelApplet *applet = UKUI_PANEL_APPLET (user_data);
	...
	if (g_strcmp0 (method_name, "PopupMenu") == 0) {
		...
		ukui_panel_applet_menu_popup (applet, button, time);
		 ...
		 gpointer         user_data)
{
	...
	if (g_strcmp0 (property_name, "PrefsPath") == 0) {
		ukui_panel_applet_set_preferences_path (applet, g_variant_get_string (value, NULL));
	} else if (g_strcmp0 (property_name, "Orient") == 0) {
		ukui_panel_applet_set_orient (applet, g_variant_get_uint32 (value));
	} else if (g_strcmp0 (property_name, "Size") == 0) {
		ukui_panel_applet_set_size (applet, g_variant_get_uint32 (value));
	} else if (g_strcmp0 (property_name, "Background") == 0) {
		ukui_panel_applet_set_background_string (applet, g_variant_get_string (value, NULL));
	} else if (g_strcmp0 (property_name, "Flags") == 0) {
		ukui_panel_applet_set_flags (applet, g_variant_get_uint32 (value));
	} else if (g_strcmp0 (property_name, "SizeHints") == 0) {
		const int *size_hints;
		gsize      n_elements;
		...
		ukui_panel_applet_set_locked (applet, g_variant_get_boolean (value));
	} else if (g_strcmp0 (property_name, "LockedDown") == 0) {
		ukui_panel_applet_set_locked_down (applet, g_variant_get_boolean (value));
	}
...
static const gchar introspection_xml[] =
	"<node>"
	  "<interface name='org.ukui.panel.applet.Applet'>"
...
static void
ukui_panel_applet_register_object (UkuiPanelApplet *applet)
{
	GError     *error = NULL;
	static gint id = 0;
	...
	applet->priv->object_path = g_strdup_printf (UKUI_PANEL_APPLET_OBJECT_PATH, applet->priv->id, id++);
	applet->priv->object_id =
...
static void ukui_panel_applet_factory_main_finalized(gpointer data, GObject* object)
{
	gtk_main_quit();
...
/*
 * To do graphical embedding in the X window system, UKUI Panel
 ...
 *
 * To solve this latter problem, UKUI Panel sets up its own X error
 ...
 */
static void
_ukui_panel_applet_setup_x_error_handler (void)
{
...
static int
_ukui_panel_applet_factory_main_internal (const gchar               *factory_id,
...
{
	UkuiPanelAppletFactory* factory;
	GClosure* closure;
	...
	if (out_process)
	{
		_ukui_panel_applet_setup_x_error_handler();
	}

	closure = g_cclosure_new(G_CALLBACK(callback), user_data, NULL);
	factory = ukui_panel_applet_factory_new(factory_id, out_process,  applet_type, closure);
	g_closure_unref(closure);

	if (ukui_panel_applet_factory_register_service(factory))
	{
		if (out_process)
		{
			g_object_weak_ref(G_OBJECT(factory), ukui_panel_applet_factory_main_finalized, NULL);
			gtk_main();
		}
...
ukui_panel_applet_factory_main (const gchar               *factory_id,
			   ...
			   gpointer                   user_data)
{
	return _ukui_panel_applet_factory_main_internal (factory_id, TRUE, applet_type,
						    callback, user_data);
}

/**
 * ukui_panel_applet_factory_setup_in_process: (skip)
 * @factory_id: Factory ID.
 * @applet_type: GType of the applet this factory creates.
 * @callback: (scope call): Callback to be called when a new applet is to be created.
 * @data: (closure): Callback data.
 ...
 */
int
ukui_panel_applet_factory_setup_in_process (const gchar               *factory_id,
				       GType                      applet_type,
				       UkuiPanelAppletFactoryCallback callback,
				       gpointer                   user_data)
{
	return _ukui_panel_applet_factory_main_internal (factory_id, FALSE, applet_type,
						    callback, user_data);
}


/**
 * ukui_panel_applet_set_background_widget:
...
#if GTK_CHECK_VERSION (3, 18, 0)
void
ukui_panel_applet_set_background_widget (UkuiPanelApplet *applet,
				    GtkWidget   *widget)
{
}
#else
				    ...
				    GtkWidget   *widget)
{
	...
	if (widget && gtk_widget_get_realized (widget)) {
		...
		cairo_pattern_t           *pattern;
		type = ukui_panel_applet_get_background (applet, &color, &pattern);
		_ukui_panel_applet_apply_css(widget,type);
...
guint32
ukui_panel_applet_get_xid (UkuiPanelApplet *applet,
		      GdkScreen   *screen)
{
...
const gchar *
ukui_panel_applet_get_object_path (UkuiPanelApplet *applet)
{
	return applet->priv->object_path;
}

G_MODULE_EXPORT GtkWidget *
ukui_panel_applet_get_applet_widget (const gchar *factory_id,
                                guint        uid)
{
	GtkWidget *widget;

	widget = ukui_panel_applet_factory_get_applet_widget (factory_id, uid);
	if (!widget) {
		return NULL;
	}

	ukui_panel_applet_setup (UKUI_PANEL_APPLET (widget));
